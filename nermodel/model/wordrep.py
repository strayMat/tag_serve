# network internal embedding functions

import torch
import torch.nn as nn
import numpy as np 

## not necessary in the first place because we will use cnn based character embeddings
	...
	def __init__(self, data):
		...
		if self.use_char:
			...
			if data.char_feature_extractor == "CNN":
				...
				self.char_feature = CharBiGRU(data.char_alphabet.size(), data.pretrain_char_embedding, self. char_embedding_dim, semf.char_hidden_dim, data.HP_dropout, self.gpu)
			elif data.char_feature_extractor == "ALL":
				self.char_all_feature = True
				self.char_feature = CharCNN(data.char_alphabet.size(), data.pretrain_char_embedding, self. char_embedding_dim, semf.char_hidden_dim, data.HP_dropout, self.gpu)
				self.char_feature_extra = CharBiLSTM(data.char_alphabet.size(), data.pretrain_char_embedding, self. char_embedding_dim, semf.char_hidden_dim, data.HP_dropout, self.gpu)
				'''
			
			else:
				print("Error char feature selection, please check parameter data.char_feature_extractor is in (**CNN**/LSTM/GRU/ALL).")
				exit(0)
	...
	def forward(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
		'''
			input: 
				...
				char_seq_lengths: list of whle batch_size for char, (batch_size*sent_len, 1)
