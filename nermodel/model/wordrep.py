...
class WordRep(nn.Module):
    ...
    def __init__(self, data):
        ...
        if self.use_char:
            ...
            if data.char_feature_extractor == "CNN":
                self.char_feature = CharCNN(data.char_alphabet.size(), 
                    data.pretrain_char_embedding, self.char_embedding_dim, self.char_hidden_dim, data.HP_dropout, self.gpu)
            ...
            ## TODO IMPLEMENT LSTM AND GRU cells for character embeddings
                ...
                print(
                    "Error char feature selection, please check parameter data.char_feature_extractor is in (**CNN**/LSTM/GRU/ALL).")
                exit(0)
        ...
        if data.pretrain_word_embedding is not None:
            ...
            self.word_embedding.weight.data.copy_(torch.from_numpy(
        ...
        for idx in range(self.feature_num):
            self.feature_embeddings.append(nn.Embedding(
        ...
        for idx in range(self.feature_num):
            if data.pretrain_feature_embeddings[idx] is not None:
                ...
                self.feature_embeddings[idx].weight.data.copy_(torch.from_numpy(self.random_embedding(
                    data.feature_alphabets[idx].size(), self.feature_embedding_dims[idx])))
        ...
        if self.gpu:
            ...
            for idx in range(self.feature_num):
                self.feature_embeddings[
                    idx] = self.feature_embeddings[idx].cuda()
    ...
    def random_embedding(self, vocab_size, embedding_dim):
        ...
        for index in range(vocab_size):
            pretrain_emb[
                index, :] = np.random.uniform(-scale, scale, [1, embedding_dim])
        return pretrain_emb
    ...
    def forward(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        ...
        if self.use_char:
            ...
            if self.char_all_feature:
                ...
                char_features_extra = char_feature_extra.view(
                    batch_size_len, -1)
